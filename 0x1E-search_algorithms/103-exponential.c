 ...
 */
size_t minimum_size(size_t a, size_t b)
{
	return (a <= b ? a : b);
}

/**
 * bisearch_helper - searches for a value in an integer array using a
...
int bisearch_helper(int *array, int value, size_t low, size_t high)
{
	size_t mid, i;
...
int exponential_search(int *array, size_t size, int value)
{
	...
	low = bound / 2;
	high = minimum_size(bound, size - 1);
	/* 'found' message generated even if array[high] < value */
	printf("Value found between indexes [%lu] and [%lu]\n", low, high);
	return (bisearch_helper(array, value, low, high));
}
